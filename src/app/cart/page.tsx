                ...
                <div className="bg-neutral-900/80 backdrop-blur-sm rounded-sm border border-white/10 overflow-hidden shadow-lg">
                  ...
                  <ul className="divide-y divide-white/10">
                    {cartItems.map((item, index) => (
                      <li key={item.id} className="p-6 flex flex-col sm:flex-row gap-4 animate-fade-in" style={{animationDelay: `${index * 100}ms`}}>
                        {/* Checkbox for item selection */}
                        <div className="flex items-center mr-2">
                          <label className="inline-flex items-center cursor-pointer">
                            <input
                              type="checkbox"
                              className="form-checkbox h-5 w-5 text-amber-500 rounded-sm border-white/20 bg-black/30 focus:ring-amber-500 focus:ring-opacity-25"
                        ...
                        {/* Product Image */}
                        <div className="sm:w-28 h-28 bg-neutral-800 rounded-sm overflow-hidden flex-shrink-0 border border-white/5 transition-transform duration-300 hover:scale-105">
                          {item.image_url ? (
                            <img 
                        ...
                        <div className="flex-grow">
                          ...
                          <p className="text-white/60 text-sm mb-2">{item.seller}</p>
                          <div className="flex flex-wrap items-center gap-4 mt-3">
                            <div className="flex items-center">
                              <button 
                                onClick={() => updateQuantity(item.id, Math.max(1, (item.quantity || 1) - 1))}
                                className="w-8 h-8 flex items-center justify-center bg-black/40 border border-white/10 text-white hover:bg-amber-500 hover:text-black transition-colors rounded-l-sm"
                              ...
                              <span className="w-12 h-8 flex items-center justify-center bg-black/20 border-t border-b border-white/10 text-white">
                                {item.quantity || 1}
                              </span>
                              <button 
                                onClick={() => updateQuantity(item.id, (item.quantity || 1) + 1)}
                                className="w-8 h-8 flex items-center justify-center bg-black/40 border border-white/10 text-white hover:bg-amber-500 hover:text-black transition-colors rounded-r-sm"
                            ...
                            <div className="text-amber-500 font-medium ml-auto">
                        ...
                        <button 
                          ...
                          className="text-white/40 hover:text-red-400 transition-colors p-2 self-start"
                  ...
                  {cartItems.length > 0 && (
                    <div className="p-4 border-t border-white/10 bg-black/20 flex justify-between items-center">
                      ...
                      <div className="text-white/60 text-sm">
                        <span>{selectedItems.size} of {cartItems.length} selected</span>
                        {selectedItems.size > 0 && (
              ...
              <div className="bg-neutral-900/80 backdrop-blur-sm rounded-sm border border-white/10 sticky top-24 shadow-lg animate-fade-in">
                <div className="p-6 border-b border-white/10 bg-black/30">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white/60 text-sm">{selectedItems.size} of {cartItems.length} selected</span>
                  </div>
                </div>
