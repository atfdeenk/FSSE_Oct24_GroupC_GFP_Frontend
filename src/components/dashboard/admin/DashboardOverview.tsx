  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        >
          <div className="text-3xl font-bold text-white mb-1">{stats?.totalTransactions || 0}</div>
          <p className="text-sm text-neutral-400">Number of transactions</p>
        </AdminCard>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <div className="w-12 h-12 bg-amber-900/30 rounded-full flex items-center justify-center mb-3 border border-amber-700/50">
              <FaUsers className="text-amber-400" size={20} />
            </div>
            <h4 className="font-medium text-white mb-1">Manage Users</h4>
            <p className="text-xs text-neutral-400">View and manage user accounts</p>
          </div>

          <div 
            className="p-4 bg-neutral-800 rounded-lg border border-neutral-700 hover:bg-neutral-700 hover:border-amber-700/50 cursor-pointer transition-all duration-200 flex flex-col items-center text-center relative shadow-md"
            onClick={() => navigateToSection('products')}
          >
            {stats && stats.pendingProducts && stats.pendingProducts > 0 && (
            ...
            <h4 className="font-medium text-white mb-1">Product Approval</h4>
            <p className="text-xs text-neutral-400">Review and approve pending products</p>
          </div>
          ...
          >
            ...
            <h4 className="font-medium text-white mb-1">Manage Balances</h4>
            <p className="text-xs text-neutral-400">Manage user wallet balances</p>
          </div>
          ...
          >
            <div className="w-12 h-12 bg-amber-900/30 rounded-full flex items-center justify-center mb-3 border border-amber-700/50">
              <FaStore className="text-amber-400" size={20} />
            </div>
            <h4 className="font-medium text-white mb-1">View Store</h4>
            <p className="text-xs text-neutral-400">See the store as customers see it</p>
          </div>
        </div>
      </AdminCard>
        ...
        <AdminCard
          ...
          footer={
            <div className="flex justify-between items-center">
              <p className="text-xs text-neutral-400">Showing {recentTransactions.length} recent transactions</p>
              <button 
                onClick={() => navigateToSection('balance')} 
          ...
          <AdminTable
            ...
            columns={[
              ...
              },
              {
                header: 'Type',
                accessor: (transaction) => (
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${transaction.type === 'credit' ? 'bg-green-900/20 text-green-400 border border-green-700/50' : 'bg-red-900/20 text-red-400 border border-red-700/50'}`}>
